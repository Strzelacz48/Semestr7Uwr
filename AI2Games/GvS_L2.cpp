...
Present select_best_target(const Gargoyle &gargoyle, vector<Present> &presents, const vector<Gargoyle> &foe_gargoyles) {
    ...
    for (auto &present : presents) {
        ...
        if(turn_counter == 1 && abs(present.x - MAX_X / 2.0) > 150){// w pierwszej turze nie rozpatrujemy oddalonych prezentów
...
int main() {
    ...
    while (1) {
        ...
        for (int i = 0; i < gargoyles_per_player; i++) {
            ...
            //if (last_target.id == target.id && last_target.x == target.x)

            if (target.id != -1) {
                cout << "FLY " << target.x << " " << target.y << " ID "<<target.id<<" "<< target.x << "," << target.y << endl;
            } else {
                cout << "FLY 960 600" << endl; // Brak odpowiedniego celu
            }
        }
    }
}