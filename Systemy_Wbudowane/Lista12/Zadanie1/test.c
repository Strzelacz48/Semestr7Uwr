...
void adc_init()
{
  ADMUX   = _BV(REFS0);// referencja 1,1V, wejście ADC1
  ADMUX  |= _BV(REFS1);
...
volatile uint16_t mesurement = 1023;
...
ISR(TIMER2_COMPA_vect)
{
    ...
    {
        mesurement = readVoltage();
        printf("\r%6u %7u %7u %7d",mesurement, target, OCR1A, v);
        i = 0;
    }
}
...
ISR(TIMER0_OVF_vect)
{
    ...
    if(i < TIME_INTERVAL)
        ...
        int16_t inputValue = pid_Controller(target << 6, mesurement << 6, &pidData);
