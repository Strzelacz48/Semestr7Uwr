...
int chekcum(uint16_t addr, uint8_t len, uint8_t &data, uint8_t checksum){
    if(1){
        return 1;
    }
    return 0;
...
int main()
{
    ...
    while(1) {
        ...
        else if (command == 'e')
        {
            ...
            uint16_t addr;
            uint8_t len;
            uint8_t type = 0,checksum;
            uint8_t data[256];
            while (!type){
                scanf(":%2hhx%4hx%2hhx",&len,&addr,&type);
                for(uint8_t i = 0; i<len;i++)
                {
                    scanf("%2hhx",&data[i]);
                }
                scanf("%2hhx",&checksum);
                printf("\r\n");
                if (chekcum(addr, len, &data, checksum)){
                    write_addr_seq()
                }
            }
        }
