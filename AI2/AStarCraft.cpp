...
pair<unsigned int, unsigned int> wrap_position(unsigned int x, unsigned int y) {
    return {x >= 19 ? x - 19 : x, y >= 10 ? y - 10 : y };
}

void print_board(vector<vector<char>> toprint) {
    for (int y = 0; y < 10; ++y) {
        for (int x = 0; x < 19; ++x) {
            cerr << toprint[y][x] << " ";
        }
...
int simulate_robot(unsigned int x, unsigned int y, char direction) {
    set<tuple<unsigned int, unsigned int, char>> visited;
    int moves = 0;
